...
// Cache roundtrip with full export pipeline: should not panic and headers must not be null
#[test]
fn cache_roundtrip_full_export() {
  if std::env::var("CACHE_EXPORT_SESSION").as_deref() == Ok("2") {
    let project_dir = PathBuf::from(std::env::var("CACHE_EXPORT_PROJECT").unwrap());
    let cache_dir = PathBuf::from(std::env::var("CACHE_EXPORT_CACHE").unwrap());
  ...
  run_child_test(
    "cache_roundtrip::cache_roundtrip_full_export",
    &[
      ("CACHE_EXPORT_SESSION", "2"),
      ("CACHE_EXPORT_PROJECT", project_dir.to_str().unwrap()),
      ("CACHE_EXPORT_CACHE", cache_dir.to_str().unwrap()),
    ],
  );
}
